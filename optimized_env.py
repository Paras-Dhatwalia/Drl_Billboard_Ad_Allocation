...
class Ad:
 ...
 self.spawn_step / 1000.0, # Temporal context [0,1]
 ], dtype=np.float32)
...
class OptimizedBillboardEnv(gym.Env):
 """
 Changed from AECEnv to gym.Env (single-agent, synchronous).
 ...
 payment_demand_ratio=float(ratio), ttl=100)
 )
 ...
 # Ad features: advertisement properties (updated: added 4 budget features)
 ...
 # Track used advertisers (each advertiser can only be used ONCE per episode)
 self.used_ad_ids: Set[int] = set()
 ...
 # Spawn new ads - only from templates that have NEVER been used this episode
 n_spawn = random.randint(*self.config.new_ads_per_step_range)
 # Check against used_ad_ids (not just current) to prevent recycling
 available_templates = [a for a in self.ads_db if a.aid not in self.used_ad_ids]
 ...
 self.used_ad_ids.add(template.aid) # Mark as used - won't spawn again
 self.performance_metrics['total_ads_processed'] += 1

 if self.config.debug:
 logger.debug(f"Spawned {spawn_count} new ads (remaining templates: {len(self.ads_db) - len(self.used_ad_ids)})")
 ...
 # Reset used advertisers (each can be used once per episode)
 self.used_ad_ids.clear()
